...
resource "null_resource" "cosmosdb_sql_role_assignment_user" {
  provisioner "local-exec" {
    command = "az cosmosdb sql role assignment list --resource-group ${azurerm_resource_group.rg.name} --account-name ${azurerm_cosmosdb_account.cosmosdb.name} --scope ${azurerm_cosmosdb_account.cosmosdb.id} --query \"[?principalId=='${data.azurerm_client_config.current.object_id}' && id=='${local.cosmosdb_role_assignment_id}'] | length(@)\" --output tsv | findstr /r /c:\"^0$\" >nul && az cosmosdb sql role assignment create --resource-group ${azurerm_resource_group.rg.name} --account-name ${azurerm_cosmosdb_account.cosmosdb.name} --role-definition-id /subscriptions/${data.azurerm_client_config.current.subscription_id}/resourceGroups/${azurerm_resource_group.rg.name}/providers/Microsoft.DocumentDB/databaseAccounts/${azurerm_cosmosdb_account.cosmosdb.name}/sqlRoleDefinitions/00000000-0000-0000-0000-000000000002 --principal-id ${data.azurerm_client_config.current.object_id} --scope ${azurerm_cosmosdb_account.cosmosdb.id} --role-assignment-id ${local.cosmosdb_role_assignment_id} || true"
  }
